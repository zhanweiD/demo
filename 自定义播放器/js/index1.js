(function () {
  ...
  //12.绑定点击音量条音量跳转事件
  // volumeProgress.addEventListener("click",function () {
  //   var volumeLeft=event.clientX-this.offsetWidth
  //   setVolume(volumeLeft)
  // })

  //11.音量控制条设置
  volumeSlide.addEventListener("mousedown",function () {
    volumeSlide.clickX=event.offsetX
  ...
  function volumeMove(){
    ...
    //计算音量条left,根据left设置volume
    var volumeLeft= event.clientX-volumeProgress.offsetLeft-volumeSlide.clickX
    setVolume(volumeLeft)

  }

    //封装音量条设置响应
  function setVolume(volumeLeft){
    //音量最大移动距离
    var volumeMax=volumeProgress.clientWidth-volumeSlide.offsetWidth
    if (volumeLeft<0){
      volumeLeft=0
    }
    else if(volumeLeft>=volumeMax){
      volumeLeft=volumeMax
    }
    volumeSlide.style.left = volumeLeft+"px"
    volumeRate.style.width = volumeLeft+"px"
    //计算比例
    var scale=volumeLeft/volumeMax
    if (scale === 0){
      videoEle.muted = true;  // 设置静音
